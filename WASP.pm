...
WASP is a means to provide structure for Web applications.  The core module
mostly provides error-handling routines, but is intended, if all components
of a Web application system use it, to provide a consistent set of behaviors
which are all robust, are easy to set up and execute, and are clean, concise,
and predictable.
...
to do so.  In this case, an administrator will only care about the behaviorial
methods and perhaps the accessor methods, to set and retrieve the behaviorial
semantics of WASP.

=head2 Programmers

Programmers using WASP, most likely for adding consistency in his or her Web
application, will only need to know of the core action methods.  The behaviorial
and accessor methods are only for refining what exactly happens when a core action
is to be executed and a programmer need not know these exact details.
...
C<die> on.  Otherwise, the value of the Boolean argument will determine
whether or not to invoke Perl's C<die> on error.
...
output on error.  In no value is given, calling this method will turn the
displaying of error messages on.  Otherwise, the value of the Boolean argument
will determine whether or not to print error messages on error.
...
This behaviorial method affects the logging of error messages on error.  The
argued file will attempt to be opened in append mode and an error message, in
the form below, will attempt to be placed at the end of the log file.

C<Cannot do something; detail1: description1; detail2: description2; detailN: ...>
...
Perhaps this method should be named "will_try_to_log." But it isn't, for hopefully
obvious reasons.
...
user-defined routines on error.  An anonymous subroutine or subroutine
reference may be given as an argument whose instructions will be processed
on error.
...
This accessor method is used to determine whether or not a delegated error-handling
routine is to be used if an error is raised.
...
This accessor method is used to determine the delegated error-handling routine that
will be invoked if an error is raised.  It will return either a reference to an
anonymous or named subroutine, or C<undef>, if no such delegate ever specified.
...
This action method performs an exception raise.  The actual semantics of what will happen
can be heavily modified by the available error-handling behaviorial methods.  By default
C<throw> will try to append to the given error message information such as the operating
system error message (e.g., as the result of failed I/O or a filesystem problem).  It will
also attempt to trace back through all subroutine calls to find the origin of the problem
(in most cases going too far to provide useful information) and when the error report was
generated.

If logging is enabled (see C<log_errors>), the error message will then attempt to be
written to the log file.  If the logging request cannot be fulfilled, additional error
information related to the failed logging attempt is appended to the error message and
further logging action is ignored.
